...
function renderUncertainty(){
 const trueV=4.10,readings=Array.from({length:6},()=>trueV+(Math.random()-.5)*.14),mean=readings.reduce((a,b)=>a+b,0)/readings.length,half=(Math.max(...readings)-Math.min(...readings))/2,pct=half/mean*100;
 $('#skillBody').innerHTML=`<div class="suite-grid"><article class="panel pad"><span class="eyebrow">Practical uncertainty simulator</span><h3>Repeated gradient measurements</h3><div class="data-mini-table">${readings.map((r,i)=>`<div><span>repeat ${i+1}</span><strong>${r.toFixed(2)}</strong></div>`).join('')}</div><p>Estimate the absolute uncertainty as half the range, then the percentage uncertainty.</p><div class="workspace-answer-row"><input id="uncAns" type="number" step="any"><span>%</span><button class="button primary" id="uncCheck">Check</button></div><div id="uncFb"></div></article><aside class="panel pad"><h3>Rules to remember</h3><ul><li>Repeated readings: a simple estimate of random uncertainty is half the range.</li><li>Percentage uncertainty = absolute uncertainty / measured value ×100.</li><li>For a power xⁿ, multiply percentage uncertainty by |n| when using the standard A-level approximation.</li><li>For graph gradients, steepest/shallowest acceptable lines give a gradient uncertainty.</li></ul></aside></div>`;$('#uncCheck').onclick=()=>{const v=+$('#uncAns').value,ok=Math.abs(v-pct)<.2;$('#uncFb').className='workspace-feedback '+(ok?'good':'bad');$('#uncFb').innerHTML=ok?'Correct.':`Expected about ${pct.toFixed(2)}%. Mean=${mean.toFixed(2)}, half-range=${half.toFixed(2)}.`};
}
...
function renderCompare(){
 const s=window.FM_SIM?.getState?.();if(!s)return;const defs=s.definition?.controls||[],key=defs[0]?.k;if(!key)return;
 const base=s.values[key],min=defs[0].min,max=defs[0].max,step=defs[0].step;const body=$('#simWorkspaceBody');body.innerHTML=`<div class="workspace-grid"><article class="workspace-card"><span class="eyebrow">Compare setup A</span><h3>${safe(s.title)}</h3><label class="field"><span>${safe(defs[0].label)}</span><input id="compareA" type="range" min="${min}" max="${max}" step="${step}" value="${base}"><output id="compareAOut">${base}</output></label><div id="compareAStats"></div></article><article class="workspace-card"><span class="eyebrow">Compare setup B</span><h3>Change one variable</h3><label class="field"><span>${safe(defs[0].label)}</span><input id="compareB" type="range" min="${min}" max="${max}" step="${step}" value="${Math.min(max,base+step*2)}"><output id="compareBOut"></output></label><div id="compareBStats"></div></article></div>`;function update(){const a=+$('#compareA').value,b=+$('#compareB').value;$('#compareAOut').textContent=a;$('#compareBOut').textContent=b;$('#compareAStats').innerHTML=compareStats(s.id,{...s.values,[key]:a});$('#compareBStats').innerHTML=compareStats(s.id,{...s.values,[key]:b})}$('#compareA').oninput=update;$('#compareB').oninput=update;update();
}
